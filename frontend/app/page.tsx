  ...
  const sendToAddress = async () => {
    try {
      if (!recipientAddress || !sendAmount) {
        uiConsole("Please enter both recipient address and amount");
        return;
      }
      ...
      const baseTransaction = {
        ...
        to: recipientAddress,
        value: amount,
        chainId: 23295, // Oasis Sapphire Testnet chain ID
      };

      // Estimate gas limit and add 20% buffer
      const estimatedGas = await web3.eth.estimateGas(baseTransaction);
      const gasLimit = (BigInt(estimatedGas) * BigInt(120)) / BigInt(100); // Add 20% buffer

      // Get current gas price
      const gasPrice = await web3.eth.getGasPrice();

      // Create final transaction with all parameters
      const transaction = {
        ...baseTransaction,
      ...
      <div>
        <div className="send-form">
          ...
          <input
            ...
            className="input-field"
          />
          <button onClick={sendToAddress} className="card">
            Send Tokens
          </button>
        </div>
      </div>
